...
int main(int argc, char *argv[])
{
	...
	/*---------------------------- GTK init ----------------------------------*/
    gtk_init(&argc, &argv);
    g_set_application_name(_("Capture Tool (video)"));
    ...
    if(!global->no_display)
    {
        ...
        gtk_window_set_title (GTK_WINDOW (gwidget->mainwin), _("Capture tool"));//GUVCViewer Controls
...
//====================================================================================================
	if (!control_only) 
	{
//----------------- Creating the xIMU thread -----------------------
		all_data.mdata->xdata->ximu_state = 0;
		
		if( __THREAD_CREATE(&all_data.ximu_thread, ximu_loop, (void *) &all_data))
		{
			g_printerr("xIMU thread creation failed\n");

			ERR_DIALOG (N_("Guvcview error:\n\nUnable to create xIMU Thread"),
				N_("Please report it to juhnowski@gmail.com"),
				&all_data);      
		}
		else 
		{
		  g_print("xIMU thread created\n");

		  if(global->ximu_timer)
		  {
			  global->ximu_timer_id=g_timeout_add(global->ximu_timer*1000, ximu_timer_callback, &all_data);
		  }
		}
		
//----------------- Creating the Encoder thread -----------------------
		if( __THREAD_CREATE(&all_data.encoder_thread, encoder_loop, (void *) &all_data))
		{
			g_printerr("Phidgets encoder thread creation failed\n");

			ERR_DIALOG (N_("Guvcview error:\n\nUnable to create Phidgets encoder Thread"),
				N_("Please report it to juhnowski@gmail.com"),
				&all_data);      
		}
		else 
		{
		  g_print("Phidgets encoder thread created\n");
		  
		  if(global->encoder_timer)
		  {
			  global->encoder_timer_id=g_timeout_add(global->encoder_timer*1000, encoder_timer_callback, &all_data);
		  }		  
		}

//----------------- Creating the Tenso thread -----------------------
		if( __THREAD_CREATE(&all_data.tenso_thread, tenso_loop, (void *) &all_data))
		{
			g_printerr("Phidgets tenso thread creation failed\n");

			ERR_DIALOG (N_("Guvcview error:\n\nUnable to create Phidgets tenso Thread"),
				N_("Please report it to juhnowski@gmail.com"),
				&all_data);      
		}
		else 
		{
		  g_print("Phidgets tenso thread created\n");
		  
		  if(global->tenso_timer)
		  {
			  global->tenso_timer_id=g_timeout_add(global->tenso_timer*1000, tenso_timer_callback, &all_data);
		  }		  
		}		
	}	
...
/*==================================================================*/
	...
	if(!control_only)
	{
	  g_print("Closing xIMU...\n");
      all_data.mdata->xdata->ximu_state = 100;

	  g_print("Closing phidgets...\n");
	  all_data.mdata->encoder_quite = TRUE;
	  all_data.mdata->tenso_quite = TRUE;
	  
	  //clean allocated metadata structures
	  freeMetaData(&all_data);
	}
