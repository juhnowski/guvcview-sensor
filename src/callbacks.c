...
/* meta controls*/
void 
set_sensitive_meta_contrls (const int flag, struct GWIDGET *gwidget)
{
	gtk_widget_set_sensitive (gwidget->XImuDeviceCB, flag);
	gtk_widget_set_sensitive (gwidget->EncoderDeviceCB, flag);
	gtk_widget_set_sensitive (gwidget->EncoderRateCB, flag);
	gtk_widget_set_sensitive (gwidget->TensoChannTB_0, flag);
	gtk_widget_set_sensitive (gwidget->TensoChannTB_1, flag);
	gtk_widget_set_sensitive (gwidget->TensoChannTB_2, flag);
	gtk_widget_set_sensitive (gwidget->TensoChannTB_3, flag);
	gtk_widget_set_sensitive (gwidget->TensoDeviceCB, flag);
	gtk_widget_set_sensitive (gwidget->TensoRateCB, flag);    
	gtk_widget_set_sensitive (gwidget->TensoRateCB, flag);
       gtk_widget_set_sensitive (gwidget->XImuPropBtn, flag);
       gtk_widget_set_sensitive (gwidget->TensoPropBtn, flag);
       gtk_widget_set_sensitive (gwidget->EncoderPropBtn, flag);
}
...
void
MetaEnable_changed (GtkToggleButton * toggle, struct ALL_DATA *all_data)
{
	struct GWIDGET *gwidget = all_data->gwidget;
	struct GLOBAL *global = all_data->global;
	
	global->Meta_enable = gtk_toggle_button_get_active (toggle) ? 1 : 0;
	if (!global->Meta_enable) 
	{
		if(global->debug) g_print("disabling meta.\n");
		set_sensitive_meta_contrls(FALSE, gwidget);
	}
	else 
	{ 
		if(global->debug) g_print("enabling meta.\n");
		set_sensitive_meta_contrls(TRUE, gwidget);
	}
...
{
    all_data->global->XImuDeviceCB_idx = gtk_combo_box_get_active (XImuDeviceCB);
}
...
{
    all_data->global->EncoderDeviceCB_idx = gtk_combo_box_get_active (EncoderDeviceCB); 
}
...
{
    all_data->global->EncoderRateCB_idx = gtk_combo_box_get_active (EncoderRateCB);
}

void
TensoDeviceCB_changed(GtkComboBox * TensoDeviceCB, struct ALL_DATA *all_data)
{
    all_data->global->TensoDeviceCB_idx = gtk_combo_box_get_active (TensoDeviceCB);
}
...
{
    all_data->global->TensoRateCB_idx = gtk_combo_box_get_active (TensoRateCB);
}
 
void
TensoChannTB_0_changed(GtkToggleButton * TensoChannTB, struct ALL_DATA *all_data)
{
    all_data->global->tenso_enabled_ch_0 = gtk_toggle_button_get_active (TensoChannTB) ? 1 : 0;
    g_printf("TensoChannTB_0_changed%i\n",all_data->global->tenso_enabled_ch_0);
}

void
TensoChannTB_1_changed(GtkToggleButton * TensoChannTB, struct ALL_DATA *all_data)
{
    all_data->global->tenso_enabled_ch_1 = gtk_toggle_button_get_active (TensoChannTB) ? 1 : 0;
    g_printf("TensoChannTB_1_changed%i\n",all_data->global->tenso_enabled_ch_1);    
}

void
TensoChannTB_2_changed(GtkToggleButton * TensoChannTB, struct ALL_DATA *all_data)
{
    all_data->global->tenso_enabled_ch_2 = gtk_toggle_button_get_active (TensoChannTB) ? 1 : 0;
    g_printf("TensoChannTB_2_changed%i\n",all_data->global->tenso_enabled_ch_2);
}

void
TensoChannTB_3_changed(GtkToggleButton * TensoChannTB, struct ALL_DATA *all_data)
{
    all_data->global->tenso_enabled_ch_3 = gtk_toggle_button_get_active (TensoChannTB) ? 1 : 0;
    g_printf("TensoChannTB_3_changed%i\n",all_data->global->tenso_enabled_ch_3);    
}
...
/*  g_print("xImu_startBtn_clicked\n");
  all_data->mdata->ximu_state = 0;
  all_data->mdata->ximu_test_quite = FALSE;
  */
}
...
/*  g_print("xImu_stopBtn_clicked\n");
  all_data->mdata->ximu_state = 0;
  all_data->mdata->ximu_test_quite = TRUE;
*/  
}
...
void
tenso_startBtn_clicked(GtkToolItem *button, struct ALL_DATA *all_data)
{
  g_print("tenso_startBtn_clicked\n");
  all_data->mdata->tenso_quite = FALSE;
  all_data->mdata->tenso_test_quite = FALSE;
  
}

void
tenso_stopBtn_clicked(GtkToolItem *button, struct ALL_DATA *all_data)
{
  g_print("tenso_stopBtn_clicked\n");
  all_data->mdata->tenso_quite = TRUE;
  all_data->mdata->tenso_test_quite = TRUE;
}

void
encoder_startBtn_clicked(GtkToolItem *button, struct ALL_DATA *all_data)
{
  g_print("encoder_startBtn_clicked\n");
  all_data->mdata->encoder_quite = FALSE;
  all_data->mdata->encoder_test_quite = FALSE;
  
}

void
encoder_stopBtn_clicked(GtkToolItem *button, struct ALL_DATA *all_data)
{
  g_print("encoder_stopBtn_clicked\n");
  all_data->mdata->encoder_quite = TRUE;
  all_data->mdata->encoder_test_quite = TRUE;
}
