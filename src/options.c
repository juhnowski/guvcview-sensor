...
writeConf(struct GLOBAL *global, char *videodevice) 
{
	...
	if ((fp = g_fopen(tmpfile,"w"))!=NULL) 
	{
		...
		g_fprintf(fp, "vcodec_flags=%d\n",vcodec_defaults->flags);
              ...
              g_fprintf(fp,"# META data\n");
              g_fprintf(fp,"# meta_enable - enable meta data\n");
              g_fprintf(fp,"meta_enable=%i\n", global->Meta_enable);

              g_fprintf(fp,"# ximu_device - ximu device index from list 0-None\n");
              g_fprintf(fp,"ximu_device=%i\n", global->XImuDeviceCB_idx);

              g_fprintf(fp,"# tenso_device - tenso device index from list 0-None\n");
              g_fprintf(fp,"tenso_device=%i\n", global->TensoDeviceCB_idx);

              g_fprintf(fp,"# tenso_rate - tenso rate device index from list\n");
              g_fprintf(fp,"tenso_rate=%i\n", global->TensoRateCB_idx);

              g_fprintf(fp,"# encoder_device - encoder device index from list 0-None\n");
              g_fprintf(fp,"encoder_device=%i\n", global->EncoderDeviceCB_idx);

              g_fprintf(fp,"# encoder_rate - encoder device rate index from list\n");
              g_fprintf(fp,"encoder_rate=%i\n", global->EncoderRateCB_idx);

              g_fprintf(fp,"# tenso_enabled_ch_0 - tenso channel 0 enabled\n");
              g_fprintf(fp,"tenso_enabled_ch_0=%i\n", global->tenso_enabled_ch_0);

              g_fprintf(fp,"# tenso_enabled_ch_1 - tenso channel 1 enabled\n");
              g_fprintf(fp,"tenso_enabled_ch_1=%i\n", global->tenso_enabled_ch_1);

              g_fprintf(fp,"# tenso_enabled_ch_2 - tenso channel 2 enabled\n");
              g_fprintf(fp,"tenso_enabled_ch_2=%i\n", global->tenso_enabled_ch_2);

              g_fprintf(fp,"# tenso_enabled_ch_3 - tenso channel 3 enabled\n");
              g_fprintf(fp,"tenso_enabled_ch_3=%i\n", global->tenso_enabled_ch_3);
...
//added META data  int settings                      
						else if (g_strcmp0(name,"ximu_device")==0)
						{
						    global->XImuDeviceCB_idx = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"tenso_device")==0)
						{
						    global->TensoDeviceCB_idx = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"tenso_rate")==0)
						{
						    global->TensoRateCB_idx = scanner->value.v_int;
						}                        
						else if (g_strcmp0(name,"encoder_device")==0)
						{
						    global->EncoderDeviceCB_idx = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"encoder_rate")==0)
						{
						    global->EncoderRateCB_idx = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"tenso_enabled_ch_0")==0)
						{
						    global->tenso_enabled_ch_0 = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"tenso_enabled_ch_1")==0)
						{
						    global->tenso_enabled_ch_1 = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"tenso_enabled_ch_2")==0)
						{
						    global->tenso_enabled_ch_2 = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"tenso_enabled_ch_3")==0)
						{
						    global->tenso_enabled_ch_3 = scanner->value.v_int;
						}
						else if (g_strcmp0(name,"meta_enable")==0)
						{
						    global->Meta_enable = scanner->value.v_int;
						}                        
						else
						{
